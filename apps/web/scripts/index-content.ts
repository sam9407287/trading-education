 ...
 *   HUGGINGFACE_API_KEY - HuggingFace API 密鑰
 */
...
async function indexContent() {
  ...
  if (!supabaseUrl || !supabaseKey) {
    console.error('❌ 錯誤：缺少 SUPABASE_URL 或 SUPABASE_SERVICE_KEY 環境變量');
    process.exit(1);
  }

  if (!hfApiKey) {
    console.error('❌ 錯誤：缺少 HUGGINGFACE_API_KEY 環境變量');
    process.exit(1);
  }
  ...
  for (const page of PAGE_CONTENTS) {
    console.log(`📄 處理頁面：${page.title} (${page.path})`);
    ...
    for (const section of page.sections) {
      ...
      try {
        // 生成 embedding
        console.log(`  - 生成向量：${section.title}`);
  ...
  // 輸出統計
  console.log('\n📊 索引完成！');
  console.log(`   總共：${totalChunks} 個內容塊`);
